...
def create_label(text: str, class_name: Optional[str] = None, style: Optional[str] = None) -> QLabel:
    lbl = QLabel(text)
    if class_name: lbl.setProperty("class", class_name)
    if style: lbl.setStyleSheet(style)
    return lbl

def create_icon_button(text: str, color_bg: str, color_text: str, tooltip: str = "") -> QPushButton:
    btn = QPushButton(text)
    btn.setFixedSize(20, 20)
    btn.setCursor(Qt.CursorShape.PointingHandCursor)
    btn.setStyleSheet(f"""
        QPushButton {{ background: {color_bg}; color: {color_text}; border-radius: 10px; border: none; font-weight: bold; }}
...
class AbilityCard(BaseCard):
    def __init__(self, ability_data: Dict[str, Any], parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.data = ability_data
        self.answers_widgets: List[Dict[str, Any]] = []
        self.init_ui()
    ...
    def init_ui(self) -> None:
        ...
        top_grid.setColumnStretch(0, 2); top_grid.setColumnStretch(1, 4); top_grid.setColumnStretch(2, 1)
        layout.addLayout(top_grid); layout.addWidget(HLine())
        ...
        layout.addLayout(extra_layout); layout.addWidget(HLine())

        # QA Area
        self._init_qa_area(layout)

    def _init_qa_area(self, parent_layout: QVBoxLayout) -> None:
        qa_frame = QFrame()
        ...
        self.practiced_cb.setStyleSheet("""
            ...
            QCheckBox::indicator:checked { background: #0055AA; }
        """)
    ...
    def _add_answer_row(self, layout: QVBoxLayout, ans_data: Dict[str, Any]) -> None:
        row = QHBoxLayout()
...
class RelationshipCard(BaseCard):
    ...
    def _init_track(self, layout: QVBoxLayout) -> None:
...
class CustomTrackCard(BaseCard):
    def __init__(self, data: Dict[str, Any], parent: Optional[QWidget] = None):
        ...
        self.data = data
        self.track_nodes: List[TrackNode] = []
        self.length: int = self.data.get("length", 15)
        self.init_ui()
    ...
    def _generate_snake_track(self) -> None:
        """生成蛇形轨道布局"""
        COLS = 15
...
class TeammateStatusWidget(QWidget):
    ...
    def update_status(self, players_list: list, my_name: str) -> None:
        # 清空旧组件
